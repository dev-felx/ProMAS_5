<form id="edit_form" class="form-horizontal" role="form">
     <div class="form-group">
    <label for="inputEnd Date" class="col-sm-4 control-label">Project Title</label>
     <div class="col-sm-7">
<input type="Project Title" class="form-control" id="inputEmail3" placeholder="Project Title">
    </div>
  </div>
    <div class="form-group">
    <label for="inputWeek No" class="col-sm-4 control-label">Semester</label>
     <div class="col-sm-7">
<input type="Semester" class="form-control" id="inputEmail3" placeholder="Semester">
    </div>
  </div>
    <div class="bottom_10 col-sm-10 col-sm-offset-1"><hr /></div>
     <div class="form-group">
    <label for="inputImplementation Methodology" class="col-sm-4 control-label">Implementation Methodology</label>
    <div class="col-sm-7">
<input type="Implementation Methodology" class="form-control" id="inputEmail3" placeholder="(Implementation Methodology) /7">
    </div>
  </div>
    <div class="form-group">
    <label for="inputSystem Functionalities" class="col-sm-4 control-label">System Functionalities</label>
    <div class="col-sm-7">
<input type="System Functionalitiest" class="form-control" id="inputEmail3" placeholder="(System Functionalities) /10">
    </div>
  </div>
     <div class="form-group">
    <label for="inputSystem Correctness" class="col-sm-4 control-label">System Correctness</label>
    <div class="col-sm-7">
<input type="System Correctness" class="form-control" id="inputEmail3" placeholder="(System Correctness) /15">
    </div>
  </div>
    <div class="form-group">
    <label for="inputPresentation Quality" class="col-sm-4 control-label">Presentation Quality</label>
    <div class="col-sm-7">
<input type="Presentation Quality" class="form-control" id="inputEmail3" placeholder="(Presentation Quality) /8">
    </div>
  </div>
    <div class="form-group">
    <label for="inputPresentation Time Compliance" class="col-sm-4 control-label">Presentation Time Compliance</label>
    <div class="col-sm-7">
<input type="Presentation Time Compliance" class="form-control" id="inputEmail3" placeholder="(Presentation Time Compliance) /5">
    </div>
  </div>
    <div class="form-group">
    <label for="inputComments" class="col-sm-4 control-label">Comments</label>
    <div class="col-sm-7">
    <textarea class="form-control" rows="3"></textarea>
    </div>
    </div>
  <div class="form-group">
    <div class="col-sm-2 col-sm-offset-9">
      <button type="Submit" class="btn btn-success">Submit</button>
    </div>
  </div>
</form>
